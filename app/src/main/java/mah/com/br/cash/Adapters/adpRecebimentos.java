...
import mah.com.br.cash.Comparators.comRecebimentos;
import mah.com.br.cash.DataBase.tblRecebimentos;
import mah.com.br.cash.Diversos.Funcoes;
import mah.com.br.cash.R;

public class adpRecebimentos extends RecyclerView.Adapter<adpRecebimentos.MyViewHolder> implements Filterable {

    public List<tblRecebimentos> mListRecebimentos;
    public int mItemSelected;
    public int mOrdem;

    private int mMes;
    private int mAno;
    private List<tblRecebimentos> mListRecebimentosFull;
    ...
    public adpRecebimentos(Context c, RecyclerView r, int mes, int ano) {
    ...
    private List<tblRecebimentos> getDados() {

        List<tblRecebimentos> mList;
    ...
    public void order() {
        ...
        comRecebimentos mComparator = new comRecebimentos(mOrdem);
    ...
    public void insert(tblRecebimentos recebimento) {
        ...
        if (mMes == recebimento.getMes() & mAno == recebimento.getAno()) {
    ...
    public void update(tblRecebimentos recebimento) {
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                try {
                    ...
                    sBuilder.append(Funcoes.addCabecalho(mContext.getString(R.string.l_007)));
                    sBuilder.append(Funcoes.addTable(mContext, mContext.getString(R.string.l_007), "700px", 1));

                    sBuilder.append("<tr>");
                    sBuilder.append(Funcoes.addTitulo(mContext, "30%", "center", "middle", mContext.getString(R.string.c_011)));
                    sBuilder.append(Funcoes.addTitulo(mContext, "15%", "center", "middle", mContext.getString(R.string.d_015)));
                    sBuilder.append(Funcoes.addTitulo(mContext, "15%", "center", "middle", mContext.getString(R.string.v_017)));
                    sBuilder.append(Funcoes.addTitulo(mContext, "35%", "center", "middle", mContext.getString(R.string.o_001)));
                    sBuilder.append("</tr>");

                    for (int i = 0; i <= mListRecebimentos.size() - 1; i++) {
                        sBuilder.append("<tr>");
                        sBuilder.append(Funcoes.addDetalhe(mContext, mListRecebimentos.get(i).getCalcNomeCliente(), "left"));
                        sBuilder.append(Funcoes.addDetalhe(mContext, mListRecebimentos.get(i).getDtRecebimento(), "center"));
                        sBuilder.append(Funcoes.addDetalhe(mContext, Funcoes.decimalFormat.format(mListRecebimentos.get(i).getVlrRecebimento()), "right"));
                        sBuilder.append(Funcoes.addDetalhe(mContext, mListRecebimentos.get(i).getObservacoes(), "left"));
                        sBuilder.append("</tr>");

                        dTotal += mListRecebimentos.get(i).getVlrRecebimento();
                    }
                    ...
                    if (tipo.matches("Relatorio")) {
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                            intent.setDataAndType(FileProvider.getUriForFile(mContext, BuildConfig.APPLICATION_ID + ".provider", Funcoes.saveFile(mContext, mContext.getString(R.string.r_007), sBuilder.toString())), "text/html");
                            intent.setFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                        } else {
                            intent.setDataAndType(Uri.fromFile(Funcoes.saveFile(mContext, mContext.getString(R.string.r_007), sBuilder.toString())), "text/html");
                    ...
                    if (tipo.matches("Compartilhar")) {
                        ...
                        intent.putExtra(Intent.EXTRA_SUBJECT, mContext.getString(R.string.l_007));
                        intent.putExtra(Intent.EXTRA_TEXT, mContext.getString(R.string.s_011));

                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                            intent.putExtra(Intent.EXTRA_STREAM, FileProvider.getUriForFile(mContext, BuildConfig.APPLICATION_ID + ".provider", Funcoes.saveFile(mContext, mContext.getString(R.string.r_007), sBuilder.toString())));
                        } else {
                            intent.putExtra(Intent.EXTRA_STREAM, Uri.fromFile(Funcoes.saveFile(mContext, mContext.getString(R.string.r_007), sBuilder.toString())));
                        }
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View v = mLayoutInflater.inflate(R.layout.list_recebimentos, viewGroup, false);
    ...
    public void onBindViewHolder(MyViewHolder myViewHolder, int i) {
        ...
        myViewHolder.txtCliente.setText(mListRecebimentos.get(i).getCalcNomeCliente());

        myViewHolder.txtData.setText(mContext.getString(R.string.d_015) +
                mContext.getString(R.string._001) + " " +
                mListRecebimentos.get(i).getDtRecebimento());

        myViewHolder.txtValor.setText(mContext.getString(R.string.v_017) +
                mContext.getString(R.string._001) + " " +
    ...
    private class DomainFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() == 0) {
                List<tblRecebimentos> list = new ArrayList<>(mListRecebimentosFull);
                results.count = list.size();
                results.values = list;
            } else {
                ArrayList<tblRecebimentos> newValues = new ArrayList<>(mListRecebimentosFull.size());

                for (int i = 0; i < mListRecebimentosFull.size(); i++) {
                    if (mListRecebimentosFull.get(i).getCalcNomeCliente().toLowerCase().contains(constraintString.toLowerCase())) {
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            ...
            if (results.values != null) {
                mListRecebimentos = (ArrayList<tblRecebimentos>) results.values;
            } else {
